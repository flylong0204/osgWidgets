...
std::unordered_map<int, int> EventAdapterWidget::key_map_ = {
...
EventAdapterWidget::EventAdapterWidget(QWidget *parent, Qt::WindowFlags f)
	: QOpenGLWidget(parent, f)
{
	device_pixel_ratio_ = devicePixelRatio();
...
EventAdapterWidget::~EventAdapterWidget()
{

}

void EventAdapterWidget::resizeGL(int w, int h)
{
	graphics_windowembedded->getEventQueue()->windowResize(0, 0, w, h);
	graphics_windowembedded->resized(0, 0, w, h);
}

void EventAdapterWidget::keyPressEvent(QKeyEvent* event)
{
	_SetKeyboardModifiers(event);
	auto found = key_map_.find(event->key());
	if (found == key_map_.end())
		return;

	graphics_windowembedded->getEventQueue()->keyPress(found->second);
}

void EventAdapterWidget::keyReleaseEvent(QKeyEvent* event)
{
	...
	else
	{
		...
		graphics_windowembedded->getEventQueue()->keyRelease(found->second);
	}
}

void EventAdapterWidget::mousePressEvent(QMouseEvent* event)
{
	int button = 0;
	switch (event->button())
	{
	...
	_SetKeyboardModifiers(event);
	graphics_windowembedded->getEventQueue()->mouseButtonPress(event->x()*device_pixel_ratio_, event->y()*device_pixel_ratio_, button);
}

void EventAdapterWidget::mouseReleaseEvent(QMouseEvent* event)
{
	int button = 0;
	switch (event->button())
	{
	...
	_SetKeyboardModifiers(event);
	graphics_windowembedded->getEventQueue()->mouseButtonRelease(event->x()*device_pixel_ratio_, event->y()*device_pixel_ratio_, button);
}

void EventAdapterWidget::mouseMoveEvent(QMouseEvent* event)
{
	_SetKeyboardModifiers(event);
	graphics_windowembedded->getEventQueue()->mouseMotion(event->x()*device_pixel_ratio_, event->y()*device_pixel_ratio_);
}

void EventAdapterWidget::mouseDoubleClickEvent(QMouseEvent *event)
{
	int button = 0;
	switch (event->button())
	{
	...
	_SetKeyboardModifiers(event);
	graphics_windowembedded->getEventQueue()->mouseDoubleButtonPress(event->x()*device_pixel_ratio_, event->y()*device_pixel_ratio_, button);
}

void EventAdapterWidget::wheelEvent(QWheelEvent* event)
{
	_SetKeyboardModifiers(event);
	graphics_windowembedded->getEventQueue()->mouseScroll(event->orientation() == Qt::Vertical ?
		(event->delta() > 0 ? osgGA::GUIEventAdapter::SCROLL_UP : osgGA::GUIEventAdapter::SCROLL_DOWN) :
		(event->delta() > 0 ? osgGA::GUIEventAdapter::SCROLL_LEFT : osgGA::GUIEventAdapter::SCROLL_RIGHT));
}

void EventAdapterWidget::_SetKeyboardModifiers(QInputEvent* event)
{
	...
	graphics_windowembedded->getEventQueue()->getCurrentEventState()->setModKeyMask(mask);
}